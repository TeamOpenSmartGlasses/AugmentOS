  ...
  private showNextDisplay(reason: 'app_stop' | 'duration_expired' | 'new_request'): void {
    ...
    if (this.displayState.backgroundLock) {
      ...
      if (expiresAt.getTime() > Date.now()) {
        // Lock still valid
        if (this.displayState.currentDisplay?.displayRequest.packageName === packageName) {
          // If lock holder is displaying, keep showing it
          return;
        }
        
        // If lock holder isn't displaying, only allow core app to show through
        ...
        return; // Still block other background apps while lock is valid
      }
