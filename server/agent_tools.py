...
def scrape_page(url: str, title: str):
    ...
    if any(substring in url for substring in banned_sites):
        ...
        try:
            ...
            return {"url": url, "description": None, "title": title.replace("|", "")}


# custom search tool, we copied the serper integration on langchain but we prefer all
# the data to be displayed in one json message
...
def serper_search(search_term: str, search_type: str = "search", **kwargs: Any) -> dict:
    headers = {
