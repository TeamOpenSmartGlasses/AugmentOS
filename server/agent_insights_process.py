...
agent = initialize_agent(
    [
        Tool(
            name="Search_Engine",
            func=custom_search,
            description="Pass this specific targeted queries and/or keywords to quickly "
            "search the WWW to retrieve vast amounts of information on "
            "virtually any topic, spanning from academic research and "
            "navigation to history, entertainment, and current events. It's "
            "a tool for understanding, navigating, and engaging with the "
            "digital world's vast knowledge.",
        ),
    ...
    while True:
        ...
        try:
            ...
            for transcript in newTranscripts:
                # DEV if len(transcript['text']) < 800: # Around 150-200 words,
                # no point to generate insight below this
                if (
                    len(transcript["text"]) < 2
                ):  # Around 150-200 words, no point to generate insight below this
                    print("Transcript too short, skipping...")
                    continue
                print(
                    "Run Insights generation with... userId: '{}' ... text: '{}'".format(
                ...
                try:
                    insight = agent.run(
                        generate_prompt(
                            f"<Old Transcript>{transcript['text'][:length-new_chunk_length]}<New Transcript>{transcript['text'][length-new_chunk_length:]}"
                        )
                    )
                    ...
                    db_handler.add_agent_insights_results_for_user(
                        transcript["userId"], [insight_obj]
                    )
                ...
                print(
                    "=== insightGeneration completed in {} seconds ===".format(
