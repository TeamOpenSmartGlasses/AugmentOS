...
class ContextualSearchEngine:
    ...
    def contextual_search_engine(self, user_id, talk):
        ...
        # run semantic search
        entities_semantic_custom = self.semantic_search_custom_data(context, user_id)
        ...
        for idx, entity in entities:
            ...
            if match_entities:
                for match_entity in match_entities:
                    ...
                    string_freq_index = self.get_string_freq(
                        whole_match.replace(":", "")
                    )
                    ...
                    if match_entity.start != 0:
                        if searched_entity[match_entity.start - 1] != " ":
                            print(
                                f"-- Skip '{whole_match}' because it's not the start of a word"
                            )
                            continue
