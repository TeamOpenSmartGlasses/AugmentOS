...
class DatabaseHandler:
    ...
    def combine_text_from_transcripts(self, transcripts, recent_transcripts_only=True):
        ...
        for index, t in enumerate(transcripts):
            if recent_transcripts_only and (
                curr_time - t["timestamp"] < self.final_transcript_validity_time
            ):
                text += t["text"]
    ...
    def get_new_cse_transcripts_for_user(self, user_id, delete_after=False):
        ...
        if user["cse_consumed_transcript_id"] != -1:
            ...
            for index, t in enumerate(user["final_transcripts"]):
                ...
                if t["uuid"] == user["cse_consumed_transcript_id"]:
                    ...
                    first_transcript["text"] = (
                        backslide_words + " " + words_from_start
                        if words_from_start
                        else backslide_words
                    )
            ...
            if (user["latest_intermediate_transcript"]["timestamp"] != -1) and (
                (time.time() - user["latest_intermediate_transcript"]["timestamp"])
                > stale_intermediate_time
            ):
    ...
    def get_recent_transcripts_from_last_nseconds_for_all_users(
        self, n=30, users_list=None
    ):
        ...
        for user in users:
            user_id = user["user_id"]
            transcript_string = (
                self.get_transcripts_from_last_nseconds_for_user_as_string(user_id, n)
            )
            if transcript_string:
                transcripts.append({"user_id": user_id, "text": transcript_string})
    ...
    def get_transcripts_from_last_nseconds_for_user(
        self, user_id, n=30, transcript_list=None
    ):
        all_transcripts = (
            transcript_list
            if transcript_list
            else self.get_all_transcripts_for_user(user_id)
        )
    ...
    def get_transcripts_from_last_nseconds_for_user_as_string(
        self, user_id, n=30, transcript_list=None
    ):
    ...
    def get_explicit_query_history_for_user(
        self, user_id, device_id=None, should_consume=True, include_consumed=False
    ):
        return self.get_results_for_user_device(
            "agent_explicit_query_ids",
            user_id,
            device_id,
            should_consume,
    ...
    def get_explicit_insights_history_for_user(
        self, user_id, device_id=None, should_consume=True, include_consumed=False
    ):
        return self.get_results_for_user_device(
            "agent_explicit_insights_result_ids",
    ...
    def get_cse_results_for_user_device(
        ...
        return self.get_results_for_user_device(
            "cse_result_ids", user_id, device_id, should_consume, include_consumed
        )
    ...
    def get_proactive_agents_insights_results_for_user_device(
        self, user_id, device_id, should_consume=True, include_consumed=False
    ):
        return self.get_results_for_user_device(
            "agent_insights_result_ids",
    ...
    def add_agent_insight_result_for_user(
        self,
        user_id,
        agent_name,
        agent_insight,
    ...
    def get_agent_proactive_definer_results_for_user_device(
        self, user_id, device_id, should_consume=True, include_consumed=False
    ):
        return self.get_results_for_user_device(
            "agent_proactive_definer_result_ids",
    ...
    def get_result_from_uuid(self, uuid):
        ...
        if res:
            return res
        res = self.agent_explicit_insights_results_collection.find_one(
            filter, {"_id": 0}
        )
        if res:
            return res
    ...
    def get_results_for_user_device(
        self,
        result_type,
        user_id,
        device_id,
        ...
        already_consumed_ids = (
            []
            if include_consumed
            else self.get_consumed_result_ids_for_user_device(user_id, device_id)
        )
        new_results = []
    ...
    def add_consumed_result_id_for_user_device(
        self, user_id, device_id, consumed_result_uuid
    ):
        filter = {"user_id": user_id, "ui_list.device_id": device_id}
        update = {"$addToSet": {"ui_list.$.consumed_result_ids": consumed_result_uuid}}
