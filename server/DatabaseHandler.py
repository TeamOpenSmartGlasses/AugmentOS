...
class DatabaseHandler:
    ...
    def save_transcript_for_user(self, user_id, text, timestamp, is_final):
        ...
        if is_final:
            ...
            if user["cse_consumed_transcript_id"] == -1:
                update = {"$set": {"cse_consumed_transcript_id": transcript["uuid"]}}
                self.user_collection.update_one(filter=filter, update=update)
        else:
            # Save to `latest_intermediate_transcript` field in database - text and
            # timestamp
            filter = {"user_id": user_id}
    ...
    def get_new_cse_transcripts_for_user(self, user_id, delete_after=False):
        ...
        if user["cse_consumed_transcript_id"] != -1:
            print()
            # Get the transcript with ID `cse_consumed_transcript_id`, get the last
            # part of it (anything after `cse_consumed_transcript_idx`)
            ...
            for index, t in enumerate(user["final_transcripts"]):
                ...
                if t["uuid"] == user["cse_consumed_transcript_id"]:
                    ...
                    # backslide
                    most_recent_final_text = (
                        user["final_transcripts"][index - 1]["text"]
                        if index > 0
                        else ""
                    )
                    previous_text_to_backslide = (
                        most_recent_final_text
                        + " "
                        + first_transcript["text"][:start_index]
                    )
                    ...
                    backslide_words = " ".join(
                        backslide_word_list[
                            -(self.backslide - len(backslide_word_list)) :
                        ]
                    )
                ...
                if first_transcript is not None:
                    # (any transcript newer than cse_consumed_transcript_id) Append any
                    # transcript from `final_transcripts` that is newer in time than
                    # the `cse_consumed_transcript_id` transcript
                    unconsumed_transcripts.append(t)

            # Append `latest_intermediate_transcript`
            if user["latest_intermediate_transcript"]["text"] != "":
                unconsumed_transcripts.append(user["latest_intermediate_transcript"])
            index_offset = 0
        else:
            # Get part `latest_intermediate_transcript` after
            # `cse_consumed_transcript_idx` index
            start_index = user["cse_consumed_transcript_idx"]
            ...
            if (len(t["text"]) - 1) > start_index:
                # backslide
                most_recent_final_text = (
                    user["final_transcripts"][-1]["text"]
                    if len(user["final_transcripts"]) > 0
                    else ""
                )
                previous_text_to_backslide = (
                    most_recent_final_text + " " + t["text"][:start_index]
                )
                backslide_word_list = previous_text_to_backslide.strip().split()
                backslide_words = " ".join(
        ...
        # Update step `cse_consumed_transcript_id` = -1 `cse_consumed_transcript_idx`
        # to index of most recent transcript we consumed in 1.
        ...
        update = {
            "$set": {
                "cse_consumed_transcript_id": -1,
                "cse_consumed_transcript_idx": new_index,
            }
        }
        self.user_collection.update_one(filter=filter, update=update)
    ...
    def get_new_cse_transcripts_for_all_users(
        self, combine_transcripts=False, delete_after=False
    ):
        users = self.user_collection.find()
        transcripts = []
        for user in users:
            ...
            if combine_transcripts:
                ...
                if transcript_string:
                    transcripts.append({"user_id": user_id, "text": transcript_string})
            else:
                transcripts.extend(
                    self.get_new_cse_transcripts_for_user(
                        user_id, delete_after=delete_after
                    )
                )
    ...
    def purge_old_transcripts_for_user_id(self, user_id):
        ...
        filter = {"user_id": user_id}
        condition = {
            "$pull": {"transcripts": {"timestamp": {"$lt": transcript_expiration_date}}}
        }
    ...
    def get_stringified_transcript_window(self, transcript_list):
        ...
        #    text_last_n_words = ' '.join(text_word_list[-(15-len(text_word_list)):])
        #    return text_last_n_words
    ...
    def stringify_transcripts(self, transcript_list):
        ...
        for i in range(last_final_transcript_index, len(transcript_list)):
            if not transcript_list[i]["is_final"]:
    ...
    def get_cse_results_for_user_device(
        self, user_id, device_id, should_consume=True, include_consumed=False
    ):
        ...
        already_consumed_ids = (
            []
            if include_consumed
            else self.get_consumed_cse_result_ids_for_user_device(user_id, device_id)
        )
    ...
    def get_agent_insights_results_for_user_device(
        self, user_id, device_id, should_consume=True, include_consumed=False
    ):
        self.add_ui_device_to_user_if_not_exists(user_id, device_id)
        ...
        already_consumed_ids = (
            []
            if include_consumed
            else self.get_consumed_agent_insights_result_ids_for_user_device(
                user_id, device_id
            )
        )
        new_results = []
    ...
    def add_consumed_cse_result_id_for_user_device(
        self, user_id, device_id, consumed_result_uuid
    ):
        filter = {"user_id": user_id, "ui_list.device_id": device_id}
        update = {
            "$addToSet": {"ui_list.$.consumed_cse_result_ids": consumed_result_uuid}
        }
        # "$add_to_set": {"ui_list": device_id}}
        self.user_collection.update_many(filter=filter, update=update)

    def add_consumed_agent_insights_result_id_for_user_device(
        self, user_id, device_id, consumed_result_uuid
    ):
        filter = {"user_id": user_id, "ui_list.device_id": device_id}
        update = {
    ...
    def get_consumed_cse_result_ids_for_user_device(self, user_id, device_id):
        ...
        if user is None or user["ui_list"] is None or user["ui_list"][0] is None:
            return []
        to_return = user["ui_list"][0]["consumed_cse_result_ids"]
        return to_return if to_return is not None else []

    def get_consumed_agent_insights_result_ids_for_user_device(
    ...
    def add_ui_device_to_user_if_not_exists(self, user_id, device_id):
        ...
        if need_add:
            ...
            ui_object = {
                "device_id": device_id,
                "consumed_cse_result_ids": [],
                "consumed_agent_insights_result_ids": [],
            }
            filter = {"user_id": user_id}
...
#
# * get_cse_results_for_user_device => REQUIRES device_id. Returns a list of CSE
# results that have not been consumed by that device_id yet. => Once this has been run,
# the same CSE result will not return again for the same device_id. => Device is
