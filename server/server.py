...
# runs when button is pressed on frontend - right now button ring on wearable or button
# in TPA
async def button_handler(request):
    ...
    user_id = body.get("userId")
    print("\n=== New Request ===\n", button_num, button_activity, timestamp, user_id)
...
# run cse/definer tools for subscribed users in background every n ms if there is fresh
# data to run on
def cse_loop():
    ...
    while True:
        ...
        try:
            ...
            for transcript in new_transcripts:
                print(
                    "Run CSE with... user_id: '{}' ... text: '{}'".format(
...
async def ui_poll(request, minutes=0.5):
    ...
    if "agent_insights" in features:
        agent_insight_results = db_handler.get_agent_insights_results_for_user_device(
            user_id=user_id, device_id=device_id
        )
