...
You are listening to a user's conversation right now. You help the language learner user by suggesting "Upgrades", contextually relevant but different words or phrases from existing vocabulary/words context in the conversation transcript (Input Text), which the user might use during the current conversation. You output 1 words in both the input language and the output language.
...
Don't pick words already in the conversation! Never suggest words in Input Text! Give an upgrade word (more advanced and related synonym, idiom, or rare word)
...
1. Skim through the Input Text and come up with a new but conversation-relevant word to someone with a fluency level of {fluency_level} AND that have not been said in the conversation.
...
#Output 4: {{ "upgrade": "gǔdài","meaning": "ancient"}}
...
# in_upgrade_translation must be in Pinyin or Latin characters.
...
#     print("LANGUAGE LEARNING WORD RANK STRING:" + word_rank_string)
    ...
    try:
        ...
        if "Pinyin" in target_language or "Pinyin" in source_language:
            ...
            translated_upgrade_words_pinyin = translated_upgrade_words_rare
        ...
        for upgrade, upgrade_meaning in translated_upgrade_words_pinyin.items():
            if upgrade not in live_upgrade_word_history_set:  # Check if word is not in the already translated words
